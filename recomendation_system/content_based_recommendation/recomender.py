...
def get_taring_set(user_id,users=None):
    ...
    if users is not None:
        ...
        all_rating=get_group_rating(all_users_rating,similarity)
    clf=RandomForestRegressor()
    x_train=m[m.anime_id.isin(x['anime_id'])]
    y=pd.merge(x_train,x,on='anime_id',how='inner')
    if users is not None:
        na_y=y[y.rating_y.isna()]
        na_y.append(all_rating)
